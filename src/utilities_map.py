...
def plot_gnss_iteration_video(curr_pose, stixel_points, dynamic_list, stixel_validity, using_prop_depth, depth_uncertainty):
    ...
    for n, (x, y) in enumerate(stixel_points_global):
        if stixel_validity[n]:
            ...
            #depth_var = depth_uncertainty[n]
            #std = np.sqrt(depth_var)

            if dynamic_list[n]:
                plt.scatter(y, x, color='red', marker='o', s=50, label="Dynamic" if 'Dynamic' not in plt.gca().get_legend_handles_labels()[1] else "")
                #plt.plot([y, y], [x - std, x + std], color='red', linewidth=2, alpha=0.6, zorder=0)
            elif using_prop_depth[n]:
                plt.scatter(y, x, color='yellow', marker='o', s=50, label="Propagated" if 'Propagated' not in plt.gca().get_legend_handles_labels()[1] else "")
                #plt.plot([y, y], [x - std, x + std], color='yellow', linewidth=2, alpha=0.6, zorder=0)
            else:
                plt.scatter(y, x, color='blue', marker='o', s=50, label="Static" if 'Static' not in plt.gca().get_legend_handles_labels()[1] else "")
                #plt.plot([y, y], [x - std, x + std], color='blue', linewidth=2, alpha=0.6, zorder=0)
...
def build_H_C_FROM_W(boat_pos, boat_quat, H_B_FROM_C):

    H_B_FROM_W           = np.eye(4)
    H_B_FROM_W[:3, :3]   = ROT.from_quat(boat_quat).as_matrix()
    H_B_FROM_W[:3, 3]    = boat_pos

    H_C_FROM_W = H_B_FROM_W @ np.linalg.inv(H_B_FROM_C)

    return H_C_FROM_W


def build_H_POINTS_W_FROM_C(boat_pos, boat_quat, H_POINTS_C_FROM_B):


    H_POINTS_W_FROM_B             = np.eye(4)
    H_POINTS_W_FROM_B  [:3, :3]   = ROT.from_quat(boat_quat).as_matrix()
    H_POINTS_W_FROM_B  [:3, 3]    = boat_pos

    #H_POINTS_W_FROM_B             = np.eye(4)
    #H_POINTS_W_FROM_B  [:3, :3]   = ROT.from_quat(boat_quat).as_matrix().T
    #H_POINTS_W_FROM_B  [:3, 3]    = -ROT.from_quat(boat_quat).as_matrix().T @ boat_pos

    H_POINTS_B_FROM_C = np.linalg.inv(H_POINTS_C_FROM_B)
    H_POINTS_W_FROM_C = H_POINTS_W_FROM_B @ H_POINTS_B_FROM_C

    #H_POINTS_W_FROM_C = H_POINTS_W_FROM_B @ H_POINTS_C_FROM_B



    return H_POINTS_W_FROM_C

def plot_previous_gnss_iterations_global(gnss_pos_list, gnss_ori_list, stixel_points_list):

    plt.figure(figsize=(8, 8))

    H_POINTS_C_FROM_B = H_POINTS_RIGHT_ZED_FROM_FLOOR
    ...
    for i, stixel_points in enumerate(stixel_points_list):
        ...
        H_POINTS_W_FROM_C = build_H_POINTS_W_FROM_C(pos, ori, H_POINTS_C_FROM_B)


        cam_pos_w = H_POINTS_W_FROM_C @ np.array([0, 0, 0, 1])
        #cam_x, cam_z = cam_pos_w[0], cam_pos_w[2]
        cam_e, cam_n = cam_pos_w[1], cam_pos_w[0]
        cam_pos_list.append((cam_e, cam_n))


        points_C = np.array([[x, 0, z, 1] for z, x in stixel_points])
        points_W = (H_POINTS_W_FROM_C @ points_C.T).T[:, [1, 0]]
        ...
        plt.scatter(xs, zs, color=color, s=10, alpha=scan_alpha, label=label)
    ...
    plt.scatter(gnss_x[-1], gnss_y[-1], color='green', marker='*', s=200, label='Camera position')
    ...
    plt.grid(True)
    plt.savefig("images/bev_consistency_200_frames_global_v6.png", dpi=300, bbox_inches='tight')
...
def plot_previous_gnss_iterations_local(gnss_pos_list, gnss_ori_list, stixel_points_list):
    ...
    curr_boat_pos = np.array([curr_boat_pos[1], curr_boat_pos[0], curr_boat_pos[2]])

    #H_B_FROM_C = H_POINTS_RIGHT_ZED_FROM_FLOOR
    H_POINTS_C_FROM_B = H_POINTS_RIGHT_ZED_FROM_FLOOR


    #H_C_CURR_FROM_W = build_H_C_FROM_W(curr_boat_pos, curr_boat_ori, H_B_FROM_C)

    H_POINTS_W_FROM_C_CURR = build_H_POINTS_W_FROM_C(curr_boat_pos, curr_boat_ori, H_POINTS_C_FROM_B)
    H_POINTS_C_CURR_FROM_W = np.linalg.inv(H_POINTS_W_FROM_C_CURR)
    ...
    for i, stixel_points in enumerate(stixel_points_list):
        ...
        #H_C_PREV_FROM_W = build_H_world_from_cam(pos, ori, H_B_FROM_C)

        #H_C_PREV_FROM_C_CURR = np.linalg.inv(H_C_CURR_FROM_W) @ H_C_PREV_FROM_W


        H_POINTS_W_FROM_C_PREV = build_H_POINTS_W_FROM_C(pos, ori, H_POINTS_C_FROM_B)
        H_POINTS_C_CURR_FROM_C_PREV = H_POINTS_C_CURR_FROM_W @ H_POINTS_W_FROM_C_PREV

        #H_POINTS_C_CURR_FROM_C_PREV = np.linalg.inv(H_POINTS_C_CURR_FROM_C_PREV)

        #if i == len(stixel_points_list) - 1:
            #print("H_POINTS_C_CURR_FROM_C_PREV", H_POINTS_C_CURR_FROM_C_PREV)


        cam_pos_local = H_POINTS_C_CURR_FROM_C_PREV @ np.array([0, 0, 0, 1])
        cam_x_local, cam_z_local = cam_pos_local[0], cam_pos_local[2]
        cam_pos_list.append((cam_x_local, cam_z_local))


        points_C_PREV = np.array([[x, 0, z, 1] for z, x in stixel_points])
        points_C_CURR = (H_POINTS_C_CURR_FROM_C_PREV @ points_C_PREV.T).T[:, [0, 2]]

        xs, zs = zip(*points_C_CURR)
        color = cmap(i/num_scans)
    ...
    plt.grid(True)
    plt.savefig("images/bev_consistency_200_frames_v28.png", dpi=300, bbox_inches='tight')
...
def plot_previous_gnss_iterations_local_2(gnss_pos_list, gnss_ori_list, stixel_points_list):
    ...
    plt.grid(True)
    plt.savefig("images/bev_consistency_200_frames_v16.png", dpi=300, bbox_inches='tight')
...
def plot_gnss_iteration_video_local(curr_pose, stixel_points, dynamic_list, stixel_validity, using_prop_depth):
    ...
    plt.scatter(0, 0, s=50, color='green', label="Camera Center")
    ...
    for n, (x, y) in enumerate(stixel_points):
        if stixel_validity[n]:
            ...
            if using_prop_depth[n]:
                ...
                plt.scatter(x, y, color='red', marker='o', s=50, label="Dynamic" if 'Dynamic' not in plt.gca().get_legend_handles_labels()[1] else "")
            else:
