...
if save_video:
    ...
    out = cv2.VideoWriter(
        f"{src_dir}/results/video_recursive_filtering_v11.mp4",
        fourcc,
        FPS,
        (width, height),
    )
...
if save_map_video:
    ...
    out_map = cv2.VideoWriter(
        f"{src_dir}/results/{ROSBAG_NAME}_video_BEV_fused_depth_cont_gate.mp4",
        fourcc,
        FPS,
        (height, height),
    )
...
def main():
    ...
    while iterating:
        ...
        stixel_img = stixels.overlay_stix   els_on_image(left_img)
